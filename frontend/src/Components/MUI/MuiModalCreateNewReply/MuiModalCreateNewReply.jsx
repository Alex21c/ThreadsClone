...
export default function MuiModalCreateNewReply({
  isItHomepage = false,
  isItSpecificThreadPage = false,
}) {
  const navigate = useNavigate();
  const [stateApiReqMessage, setStateApiReqMessage] = useState(
    "Posting Yours Reply..."
  );
  const auth = useSelector((store) => store.auth);
  const user = useSelector((store) => store.user);
  const threads = useSelector((store) => store.threads);
  const muiModalCreateNewReply = useSelector(
    (store) => store.muiModalCreateNewReply
  );
  ...
  useEffect(() => {
    debouncedHandleTextareaChange(
      refTextarea,
      refDivVerticalLine,
      refImageUploadByUser
    );
  ...
  function handleTextareaChange(
    ...
    try {
      ...
      const previewImageScrollHeight = refImageUploadByUser?.current
        ? refImageUploadByUser.current.scrollHeight
        : 0;
      const decrement = refTextarea.current.scrollHeight > 100 ? 10 : 10;

      refDivVerticalLine.current.style.height =
        Number(
          refTextarea.current.scrollHeight +
            previewImageScrollHeight -
            decrement
        ) + "px";
    } catch (error) {
  ...
  const debouncedHandleTextareaChange = useCallback(
    Utils.debounce(
      (refTextarea, refDivVerticalLine, refImageUploadByUser) =>
  ...
  async function handReqCreateANewReply(replyBelongsToThisThreadID = null) {
    ...
    if (bodyText === "") {
      return dispatch(
        openTheMuiSnackbar({
          message: "Kindly provide reply Text",
          type: "info",
        })
      );
    } else if (!replyBelongsToThisThreadID) {
      console.log(
        "required field is missing : replyBelongsToThisThreadID in params"
      );
      return;
    }
    ...
    try {
      ...
      dispatch(
        openTheMuiSnackbar({
          message: "Unable to Post a Reply, please try again after some time!",
          type: "error",
        })
      );
    } finally {
        ...
        >
            ...
            <div className="flex gap-[2rem]">
              <div className="flex items-center flex-col w-[10%]">
                <div className="w-[3rem]  overflow-hidden">
                  <img
                    src={
                      muiModalCreateNewReply.replyingToThisThread?.createdBy
                        ?.profileImage?.url
                    }
              ...
              <div className="w-[90%] flex flex-col gap-[1rem]">
                <div className="flex gap-[1rem]">
                  ...
                  <span style={{ color: theme.secondaryText }}>
                    {Utils.getRelativeTime(
                      muiModalCreateNewReply.replyingToThisThread?.createdAt ||
                        null
                    )}
                  </span>
                </div>
                ...
                {muiModalCreateNewReply.replyingToThisThread?.bodyImage
                  ?.url && (
          ...
          <div className="flex gap-[2rem]">
            <div className="flex items-center flex-col w-[10%]">
              ...
              <div
                ref={refDivVerticalLine}
                className="w-[.1rem] h-[2rem] my-[1rem]"
                style={{ backgroundColor: theme.borderColor }}
              ></div>
            ...
            <div className="w-[90%] flex flex-col gap-[1rem]">
              ...
              {statePosting ? (
                ...
                <button
                  className="px-[1rem] py-[.5rem] border-[.1rem] rounded-xl w-[5rem] self-end"
                  style={{
                    borderColor: theme.borderColor,
                    color: theme.primaryText,
                    backgroundColor: theme.background,
