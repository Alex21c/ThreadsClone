...
export const fetchRepliesMadeByCurrentUser = createAsyncThunk(
  "replies/fetchRepliesMadeByCurrentUser",
  async (auth) => {
    try {
      // safeguard
      ...
      if (response?.statusText === "Unauthorized") {
        ...
        localStorage.removeItem(
          process.env.REACT_APP_PREFIX_LOCALSTORAGE + "Authorization"
        );
      ...
      console.error(
        "ThredsCloneCustomError: failed to make fetch req redux thunk " +
          error.message
      );
    }
  }
...
export const fetchSpecificUserReplies = createAsyncThunk(
  "replies/fetchSpecificUserReplies",
  async ({ auth, username }) => {
    try {
      // safeguard
      if (!auth.authorization) {
...
const repliesMadeByCurrentUserFetchedFromLocalStorage = localStorage.getItem(
  process.env.REACT_APP_PREFIX_LOCALSTORAGE + "repliedMadeByCurrentUser"
);
...
export const replySlice = createSlice({
  ...
  initialState: {
    specificUserReplies: [],
    repliesMadeByCurrentUser: repliesMadeByCurrentUserFetchedFromLocalStorage
      ? JSON.parse(repliesMadeByCurrentUserFetchedFromLocalStorage)
      : [],
  },
  reducers: {
...
// exporting actions
export const { setRepliesMadeByCurrentUser, clearRepliesData } =
  replySlice.actions;
