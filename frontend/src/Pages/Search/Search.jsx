...
export default function Search() {
  ...
  useEffect(() => {
    ...
    stateDebouncedMakeApiCall.current = Utils.debounce(
      (query) => makeApiCallFetchUsersOrThreadsMatchingSearchQuery(query),
      1000
    );
  ...
  async function makeApiCallFetchUsersOrThreadsMatchingSearchQuery(
    query = null
  ) {
    try {
      if (!query) {
      ...
      >
        <div className="flex justify-between items-center relative">
          <input
            onChange={(event) => {
              ...
              stateDebouncedMakeApiCall.current &&
                stateDebouncedMakeApiCall.current(event.target.value);
            }}
            ref={refInputSearch}
            style={{
              color: theme.primaryText,
          ...
          <i
            ...
            className="fa-regular fa-magnifying-glass absolute left-[1rem]"
          ></i>
        </div>
          ...
          {stateQuery !== "" && (
            <>
              ...
              <div className="flex gap-[1rem] relative">
                ...
                <i
                  style={{ color: theme.secondaryText }}
                  className="fa-solid fa-chevron-right absolute right-[1rem]"
                ></i>
              </div>
              <div className="py-[1rem]">
                <Divider
        ...
        <div className="flex flex-col gap-[1rem]">
          {stateSearchQueryResponse?.users?.length > 0 &&
            stateSearchQueryResponse.users.map((otherUser) => (
              <div
                key={otherUser._id}
                className="flex  justify-between"
                id="wrapperUserIconUserNameAndFollowBtn"
              >
                <div className="flex gap-[1rem]">
                  <Link
                    to={`/profile/${otherUser?.username}`}
                    className="w-[4rem] h-[4rem] rounded-full overflow-hidden"
                  >
                    <img
                  ...
                  <div className="flex flex-col gap-[.5rem]">
                    <div className="relative">
                      ...
                      <div
                        onMouseLeave={() => {
                          setTimeout(
                            () => setStateIsUserCardOpened(false),
                            300
                          );
                        }}
                        className={`${
                          stateIsUserCardOpened === true &&
                          stateUserName === otherUser?.username
                            ? "flex"
                            : "hidden"
                        }  absolute z-[100] top-[1.2rem] left-[-1.5rem]   p-[2rem] rounded-xl w-[25rem] flex-col gap-[1rem]`}
                        style={{ backgroundColor: theme.background }}
                      >
                        <Link
                          to={`/profile/${otherUser?.username}`}
                          className="flex gap-[1rem] w-[100%] cursor-pointer"
                        >
                    ...
                    <span
                      style={{ color: theme.secondaryText }}
                    >{`${otherUser?.firstName} ${otherUser?.lastName}`}</span>
        ...
        <div className="flex flex-col gap-[1rem]">
          {stateSearchQueryResponse?.threads?.length > 0 &&
            stateSearchQueryResponse.threads.map((thread, idx) => (
              <Thread
                key={thread._id}
                threadData={thread}
                totalItems={stateSearchQueryResponse.threads?.length}
                idx={idx}
              />
            ))}
        </div>
