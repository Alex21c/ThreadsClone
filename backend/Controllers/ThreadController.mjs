...
const getHomepageThreadsForCurrentUser = async(req, res, next)=>{
  try {
    ...
    req.user.following.forEach(user=>{
      query.push({ createdBy: user._id, replyBelongsToThreadCreatedByThisUser: null })
    })
...
const ThreadController = {createNewThread, likeAThread, unlikeAThread, deleteAThread, getAllTheThreadsCreatedByCurrentUser, getHomepageThreadsForCurrentUser, getSpecificThread, getAllTheThreadsCreatedBySpecificUser};
